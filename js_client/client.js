...
function handleLogin(event) {
    ...
    .then(authData => {
        handleAuthData(authData, getLoanList)
    })
    .catch(err=> {
        console.log('err', err)
    })
...
function getLoanList(){
    const endpoint = `${baseEndpoint}/loans/`
...
const searchClient = algoliasearch('838O61C158', 'df8100b5a35c591080e9b46637e87eb2');
...
search.addWidgets([
  ...
  instantsearch.widgets.hits({
    ...
    templates: {
        item: `
            <div>
                <div>{{#helpers.highlight}}{ "attribute": "bank_id" }{{/helpers.highlight}}</div>
                <div>{{#helpers.highlight}}{ "attribute": "body" }{{/helpers.highlight}}</div>
                
                <p>{{ user }}</p><p>{{ loan_requirement }}
